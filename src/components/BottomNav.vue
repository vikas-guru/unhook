...
<template>
  ...
  >
    <div class="mx-auto flex max-w-2xl items-stretch justify-around px-2">
      ...
      >
        <span class="text-lg leading-none transition-transform group-hover:-translate-y-0.5">{{ t.icon }}</span>
        <span>{{ t.label }}</span>
      </RouterLink>
      ...
      <button
        ...
        @click="openSOS"
      >
        <span class="grid h-14 w-14 place-items-center rounded-full bg-[var(--accent)] text-2xl text-white shadow-[0_10px_30px_-6px_rgba(224,120,90,.6)] ring-4 ring-[var(--bg)] transition active:scale-95">🆘</span>
